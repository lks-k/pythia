...
@model function bayes_hidden_markov_model(y, K)
    ...
    # y[(0) + 1] ~ Normal(m[(s[(0) + 1]) + 1], 0.1)  # FIXME: Float indexing is not allowed.
    y[(0) + 1] ~ Normal(m[(trunc(Int, s[(0) + 1])) + 1], 0.1)
    for i = 1:1:(length(y))-1
        s[(i) + 1] ~ DiscreteUniform(0, (K) - (1))
        # y[(i) + 1] ~ Normal(m[(s[(i) + 1]) + 1], 0.1)  # FIXME: Float indexing is not allowed.
...
# FIXME: Doesn't fit well (?), though I'm not sure how to properly test this.
